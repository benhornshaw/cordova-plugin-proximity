...
public class Proximity extends CordovaPlugin implements SensorEventListener{
    ...
    float distanceRead;					//acceleration in x axis
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException{
    	if (action.equals("start")){
    		...
    		if (this.status != Proximity.RUNNING){
    			int val = this.start();
    			Log.i("getReading","In the function");

    			if(val == Proximity.FAILED_TO_START){
    				Log.e("getReading","Failed to start");
    				callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.IO_EXCEPTION, Proximity.FAILED_TO_START));
    				return false;
    			}
	...
	public int start(){
		...
		if (list != null && list.size()>0){
			...
			this.setStatus(Proximity.FAILED_TO_START);
		}
   ...
   /**
    ...
    private JSONObject getReading() throws JSONException {
    	...
    	obj.put("distance", this.distanceRead);
